...
C10_HOST_DEVICE static void reduce_fraction(
    size_t& numerator,
    size_t& denominator) {
...
struct ReduceConfig {
  ...
  dim3 grid() const {
    return dim3(
        div_up(num_outputs / output_vec_size, step_output), ctas_per_output);
  }
  ...
  C10_HOST_DEVICE int input_idx() const {
    ...
    int cta2 = blockIdx.y;
    return (
        lane * input_mult[BLOCK_X] + warp * input_mult[BLOCK_Y] +
        cta2 * input_mult[CTA]);
  }
  ...
  int shared_memory_size() const {
    if (!should_block_y_reduce() &&
        (!should_block_x_reduce() || block_width <= at::cuda::warp_size())) {
      return 0;
    }
...
struct ReduceJitOp {
  // ReduceJitOp is almost like ReduceOp, but it doesn't have ops functor that
  // specifies reduction operations Maybe we can find a way to unify ReduceOp
  // and ReduceJitOp
  using InputCalculator = OffsetCalculator<1, uint32_t>;
  using OutputCalculator = OffsetCalculator<2, uint32_t>;
  // TODO for now arg_t is always opmath_t of the input, later we'll need to
  // change it
  using arg_t = at::opmath_type<scalar_t>;

  static constexpr int input_vec_size = ReduceConfig::input_vec_size;
  // TODO - ReduceJitOp will probably need to be changed for reductions that
  // need full functor, not just wrapper
  arg_t ident;
  ReduceConfig config;
...
#pragma unroll
  ...
  template <int output_vec_size>
  C10_DEVICE at::detail::Array<arg_t, output_vec_size> thread_reduce(
  ...
  input_vectorized_thread_reduce_impl(const scalar_t* data) const {
    ...
    if (shift > 0) {
      ...
      if (threadIdx.x >= shift && threadIdx.x < align_elements &&
        ...
        value = ops.reduce(
            value, c10::load(data + threadIdx.x), threadIdx.x - shift);
      }
      end -= align_elements;
      data += align_elements;
...
#pragma unroll
      for (index_t i = 0; i < input_vec_size; i++) {
        value_list[i] = ops.reduce(
            value_list[i], values_vec.val[i], shift + idx * input_vec_size + i);
      }
      idx += stride;
    }
...
#pragma unroll
  ...
  C10_DEVICE at::detail::Array<arg_t, output_vec_size> thread_reduce_impl(
...
#pragma unroll
  ...
  template <int output_vec_size>
  C10_DEVICE void set_results_to_output(
      at::detail::Array<arg_t, output_vec_size> value,
...
#pragma unroll
  ...
  template <int output_vec_size>
  C10_DEVICE at::detail::Array<arg_t, output_vec_size> global_reduce(
      at::detail::Array<arg_t, output_vec_size> value,
...
inline void launch_jitted_reduce_kernel(
  ...
  at::cuda::jit::launch_jitted_pwise_function(
      *fn_ptr, args, grid, block, shared_memory);
}
...
ReduceConfig setReduceConfig(const TensorIterator& iter) {
  ...
  if (config.values_per_thread() >= block_height * min_values_per_thread ||
...
// TODO this is 100 lines of almost-copy-paste, because we have to have
// different template args for this function try unifying with gpu_reduce_kernel
template <
    char const* name,
    typename scalar_t,
    typename out_scalar_t,
    int vt0 = 4,
    typename ident_t = double>
inline void jitted_gpu_reduce_kernel(
    TensorIterator& iter,
  ...
  // TODO - this will be different for more complicated reductions, but for now
  // reductions using func_wrapper all have arg_t = opmath
  using arg_t = at::opmath_type<scalar_t>;
  // at::Half/at::ComplexHalf overflows easily as it's range is very small.
  // So when scalar_t and out_scalar_t are at::Half/at::ComplexHalf, we
  // set can_accumulate_in_output to False.
  ...
  if (!can_use_32bit_indexing) {
    for (auto& sub_iter : iter.with_32bit_indexing()) {
      ...
      jitted_gpu_reduce_kernel<name, scalar_t, out_scalar_t, vt0>(
          sub_iter, func, ident, acc_buf_ptr, sub_iter_base_idx);
    }
    return;
  }

  // TODO - for now we support a single input, we may be able to relax this
  // constraint
  const char* in_data = (char*)iter.data_ptr(iter.ntensors() - 1);
  char* out_data = (char*)iter.data_ptr(0);
  const auto noutputs = iter.noutputs();
  ...
  static auto desc =
      at::cuda::jit::make_kernel_descriptor<out_scalar_t, scalar_t>(
          name, func, nInputs, nOutputs);

  static std::mutex jiterator_mutex;
  static std::vector<std::array<at::cuda::jit::NvrtcFunction, 3>> fn_cache(
