...
def run_selected_tests(workflow_name, test_run_test_path, overall_logs_path_current_run, test_reports_src, selected_list, exclude_tests, exclude_core):
    ...
    if workflow_name == "default":
        ...
        command = f"python {test_run_test_path} --include {default_selected_test_suites} --exclude-jit-executor --exclude-distributed-tests --verbose"
        ...
        command = f"python {test_run_test_path} --include {distributed_selected_test_suites} --distributed-tests --verbose"
        run_command_and_capture_output(command)
        del os.environ['HIP_VISIBLE_DEVICES']
    elif workflow_name == "inductor":
        os.environ['TEST_CONFIG'] = 'inductor'
        ...
        if inductor_selected_test_suites:
            command = f"python {test_run_test_path} --include {' '.join(inductor_selected_test_suites)} --verbose"
            run_command_and_capture_output(command)
        if non_inductor_selected_test_suites:
            command = f"python {test_run_test_path} --inductor --include {' '.join(non_inductor_selected_test_suites)} --verbose"
            run_command_and_capture_output(command)
...
def run_test_and_summarize_results(
    ...
    if not priority_tests and not default_list and not distributed_list and not inductor_list:
        ...
        if inductor_list:
            res_inductor_selected = run_selected_tests("inductor", test_run_test_path, overall_logs_path_current_run, test_reports_src, inductor_list, exclude_tests, exclude_default_core)
